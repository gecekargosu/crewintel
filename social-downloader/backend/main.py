...
yt-dlp ile YouTube, Instagram, TikTok, Facebook, Pinterest videolarını indirir.
"""
...
def get_download_url(url: str, quality: str = "best", format_type: str = "video") -> dict:
    ...
    return {
        ...
        "url": lines[1] if len(lines) > 1 else url,
        "duration": lines[2] if len(lines) > 2 else None,
        "filesize": lines[3] if len(lines) > 3 else None,
    }

# ── API Endpoints ─────────────────────────────────────────────────────────────
@app.get("/health")
async def health():
    return {"status": "ok", "service": "social-downloader"}
